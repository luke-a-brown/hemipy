# HemiPy: A Python module for automated and traceable estimation of forest biophysical variables from digital hemispherical photographs

## Introduction

`hemipy` is an open source Python module for deriving forest biophysical variables from digital hemispherical photographs in an automated and traceable manner. `hemipy` is well-suited to batch processing, supports a wide range of image formats, and includes propagation and provision of uncertainties. 
...
<sup>[2]</sup>Two alternative apprpoaches are implemented to estimate PAI<sub>e</sub>, in which a random distirbution of leaves is assumed: Warren-Wilson's (1963) approach, which considers gap fraction at the hinge region surrounding the zenith angle of 57.5° only (where gap fraction is nearly independent of leaf angle distribution), and a generalised version of Miller's (1967) integral, which makes use of a fuller range of multi-angular observations. In both cases, to account for the effects of foliage clumping and derive PAI as opposed to PAI<sub>e</sub>, Lang and Yueqin's (1986) logarithm averaging method is adopted. Ω is computed as the ratio of PAI<sub>e</sub> to PAI.

<sup>[3]</sup>For downwards-facing images, effective green area index (GAI<sub>e</sub>) and green area index (GAI) are provided as opposed to PAIe and PAI. 

## Installation

The latest release of `hemipy` can easily be installed using `pip`:

`pip install https://github.com/luke-a-brown/hemipy/archive/refs/tags/v0.1.0.zip`
...
* Use the zenith rings defined in the computation of PAI according to Miller (1967) for deriving FIPAR, FCOVER, and PAI according to Warren-Wilson (1963) (`use_miller_rings`, default is False, i.e. to use dedicated rings centred at the solar zenith angle, nadir, and 57.5°, which is more accurate, but will increase computation time). If True, the rings with the closest central zenith angle are used;
...
* Apply a mask of a specified size to the bottom of the image (`mask`, useful for removing the operator’s legs in downwards-facing images, default is a 90° mask);
...
The example below demonstrates how `hemipy` can be used to process a set of images from multiple measurement plots with the following directory structure:
...
import numpy as np
import math, hemipy, glob, exifread

#define input directory (sub-directories correspond to measurement plots and contain images from that plot)
input_dir = 'example_data/'
#define latitutde of the site (necessary for FIPAR computation)
lat = 45.5045929

#define image size and optical centre (assumed equal to image centre as no calibration information available
img_size = np.array([4032, 6032])
opt_cen = np.array([img_size[0] / 2, img_size[1] / 2])
#define calibration function coefficients (^3, ^2 and ^1) (linear function over the diagonal assumed for full-frame fisheye)
cal_fun = np.array([0, 0, 180.0 / math.sqrt(img_size[0] ** 2 + img_size[1] ** 2)])
...
output_file.write('Date,Plot,Direction,PAIe_Warren,PAI_Warren,Clumping_Warren,
...
for i in range(len(plots)):
    ...
    for j in range(len(layers)):       
        ...
        image = open(glob.glob(layers[j]+'/*.nef')[0], 'rb')
        tags = exifread.process_file(image)
        #determine date and time of plot acquisition
        date = str(tags['EXIF DateTimeOriginal'])[0:10].replace(':', '-')
        time = str(tags['EXIF DateTimeOriginal'])[-8:]
        ...
        results = hemipy.process(layers[j], zenith, azimuth, date = date, direction = direction,
                                 lat = lat)
                                 
        output_file.write(date + ',' +\
                          layers[j].split('\\')[1] + ',' +\
                          direction + ',' +\
                          str(results['paie_warren']) + ',' +\
                          str(results['pai_warren']) + ',' +\
                          str(results['clumping_warren']) + ',' +\
...
Chianucci, F., 2013. Canopy Properties Estimation in Deciduous Forests with Digital Photography. Università degli Studi della Tusci.
